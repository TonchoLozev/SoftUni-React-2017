...
const dataService = {
    ...
    getBookById: (id) => {
        return new Promise((res, rej) => {
            res(allBooks.filter(b => id === b.id)[0])
        })
    },
    getAllAuthors: () => {
        return new Promise((res, rej) => {
    ...
    getAuthorById: (id) => {
        return new Promise((res, rej) => {
            res(allAuthors.filter(a => a.id === id)[0])
        })
    },
    getComments: (commentsArr) => {
        return new Promise((res, rej) => {
            ...
            for (let comment of allComments) {
                if (commentsArr.includes(comment.id)) {
...
//dataService.getAllBooks().then(books => console.log(books));
//dataService.getBookById(2).then((book) => {console.log(book)});
//dataService.getAllAuthors().then(authors => console.log(authors));
//dataService.getAuthorById(1).then(author => console.log(author));
//dataService.getComments([4, 5]).then(comments => console.log(comments));
